...
subscribe_topic="runMES/qry_dcplan_item_srv"
srv_name='mq_qry_dcplan_item_srv'
...
def on_message(client,userdata,msg):
  ...
  try:
    log_EAP_IF.to_info({'MQTT':srv_name,'STATUS':'on-message','TOPIC':msg.topic,'MSG':msg.payload})
    payload=bytes.decode(msg.payload)
    #payload=msg.payload.decode('utf8')
    #log_EAP.to_info({'MQTT':srv_name,'STATUS':'on-message','payload bytes decode':payload})
    ...
    plan=d['DCPLAN_TXT']
    op=d['OP_TXT']
    log_EAP_IF.to_debug({'MQTT':srv_name,'STATUS':'on-message parse','TID':tid,'RTN':rtn,'DCPLAN':plan,'OP':op})

    # qry_dcplan_item(dcplan_name_txt,op_txt)
    reply=trans.qry_dcplan_item(plan,op)
    ...
    mylog.exception(e)
    mylog.error({'MQTT':srv_name,'STATUS':'on-message','ERR':e})
