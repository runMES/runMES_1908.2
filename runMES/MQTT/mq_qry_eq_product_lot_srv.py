...
subscribe_topic="runMES/qry_eq_product_lot_srv"
srv_name='mq_qry_product_lot_srv'
...
def on_message(client,userdata,msg):
  ...
  try:
    ...
    rtn=d['RTN_TXT']
    eq=d['EQ_TXT']
    product=d['PRODUCT_TXT']
    op=d['OP_TXT']
    log_EAP_IF.to_info({'MQTT':srv_name,'STATUS':'on message','TID':tid,'RTN':rtn,'EQ':eq,'PRODUCT':product,'OP':op})

    #
    reply=trans.qry_eq_product_query_lot(eq,product,op)
    msg={'TID_TXT':tid,'RTN_TXT':rtn,'RPY_TXT':reply}
    log_EAP_IF.to_debug({'MQTT':srv_name,'STATUS':'tns reply','msg':msg})
