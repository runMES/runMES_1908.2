...
def on_connect(client,userdata,flags,rc):
  ...
  #log_EAP_IF.to_debug({'MQTT':srv_name,'STATUS':'on_connect','RC':rc,'CLIENT':client,'USERDATA':userdata,'FLAGS':flags})
...
def on_message(client,userdata,msg):
  ...
  try:
    ...
    # log_EAP_IF.to_debug({'payload':payload})
    ...
    items=d['ITEM_SET']
    anno=d['ANNO_TXT']
    log_EAP_IF.to_debug({'MQTT':srv_name,'STATUS':'on-message','TID':tid,'RTN':rtn,'LOT':lot,'EQ':eq,'DCPLAN':dc_plan,'OP':op,'ITEM_SET':items,'ANNO':anno})

    # def tx_dc(lot_txt,eq_txt,dcplan_txt,item_set,op_txt,anno_txt):
    reply=trans.tx_dc(lot,eq,dc_plan,items,op,anno)
