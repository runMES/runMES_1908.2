...
subscribe_topic="runMES/qry_eq_record_srv"
srv_name='mq_qry_eq_record_srv'
...
def on_connect(client,userdata,flags,rc):
  ...
  #log_EAP.to_debug({'MQTT':'mq_qry_eq_record_srv on_connect','RC':rc,'CLIENT':client,'USERDATA':userdata,'FLAGS':flags})
...
def on_message(client,userdata,msg):
  ...
  try:
    log_EAP_IF.to_info({'MQTT':'mq_qry_eq_record_srv<','TOPIC':msg.topic,'MSG':msg.payload})
    payload=bytes.decode(msg.payload)
    #payload=msg.payload.decode('utf8')
    #log_EAP.to_debug({'MQTT':'mq_qry_eq_record_srv-on_message','payload bytes decode':payload})
    ...
    eq=d['EQ_TXT']
    op=d['OP_TXT']
    log_EAP_IF.to_debug({'MQTT':srv_name,'STATUS':'on message','TID':tid,'RTN':rtn,'EQ':eq,'OP':op})

    # qry_eq_record(eq_txt,op_txt)
    reply=trans.qry_eq_record(eq,op)
